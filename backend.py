...
def eh_evento_reservado(nome_evento: str, constantes: dict) -> bool:
    """Verifica se o título do evento corresponde a um evento reservado."""
    if not nome_evento:
        return False
...
def carregar_agendas(arquivo_json: str = "agendas.json") -> dict:
    ...
    try:
        ...
        st.error("❌ Arquivo 'agendas.json' não encontrado.")
    except json.JSONDecodeError:
...
# ============================================================
#               CARREGAMENTO DE EVENTOS (ICS)
# ============================================================

@st.cache_data(ttl=60)
def carregar_eventos(url: str, dias_a_frente: int) -> list:
    """Baixa e padroniza eventos de uma URL ICS."""
    ...
    try:
        ...
        st.error(f"Falha ao carregar a agenda. Erro: {e}")
        return []


# ============================================================
#    ENCONTRAR HORÁRIOS EM QUE TODOS ESTÃO EM EVENTOS PET
# ============================================================

def encontrar_horarios_pet_comuns(eventos_por_membro: dict, intervalo_min: int, dias: int) -> list:
    """
    Retorna horários onde TODOS os membros têm evento PET
    e não há conflito com eventos reservados.
    """
    ...
    while tempo_atual < fim_periodo:
        ...
        if 7 <= tempo_atual.hour < 22:
            ...
            for _, eventos in eventos_por_membro.items():
                ...
                # Verificar se o membro está em PET
                if not any(
...
# ============================================================
#           CÁLCULO DE HORÁRIOS LIVRES INDIVIDUAIS
# ============================================================

def calcular_horarios_livres(eventos_todos: list, intervalo_min: int, dias: int) -> list:
    """
    Determina horários livres considerando apenas eventos reservados.
    Analisa apenas entre 7h30 e 22h.
    """
    CONSTANTES = carregar_constantes()
    horarios_livres = []
            ...
            for e in eventos_todos:
                if e["inicio"] <= tempo_atual < e["fim"]:
                    ...
                    if eh_evento_reservado(e.get("nome", ""), CONSTANTES):
                        ...
                        # Se o evento NÃO é reservado, também conta como ocupado
                        ocupado = True
